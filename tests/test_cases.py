...
def test_foreign_player_count(load_team_data):
    ...
    assert player_count == 4, "Team cannot have more or less than 4 foreign players"    # validating foreign players count
    print("Team has only 4 foreign players")


def test_one_keeper(load_team_data):
    player_count = count_wicketkeeper(load_team_data)
    assert player_count >= 1, "Team should have at least one WK"    # validating wicketkeeper count
